...
class AuditRepositoryTests(unittest.TestCase):
    def test_scores_repository_health_signals(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            (root / ".github").mkdir()
            (root / ".github" / "labels.yml").write_text("- name: bug\n", encoding="utf-8")
        ...
        self.assertTrue(checks["support"].passed)
        self.assertTrue(checks["maintainers"].passed)
        self.assertTrue(checks["labels"].passed)
        self.assertTrue(checks["ci"].passed)
        self.assertTrue(checks["release_workflow"].passed)
        self.assertTrue(checks["roadmap"].passed)
